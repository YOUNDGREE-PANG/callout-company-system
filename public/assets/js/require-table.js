define(['jquery', 'bootstrap', 'moment', 'moment/locale/zh-cn', 'bootstrap-table', 'bootstrap-table-lang', 'bootstrap-table-export', 'bootstrap-table-commonsearch', 'bootstrap-table-template', 'bootstrap-table-jumpto', 'bootstrap-table-fixed-columns'], function ($, undefined, Moment) {
    var Table = {
        ...
        config: {
            ...
            dragsortfield: 'weigh',
            addtobtn:'.btn-addto',
            addtothebtn:'.btn-addtothe',
            distbtn: '.btn-dist',
            getoutbtn:'.btn-getout',
            recythisbtn:'.btn-recythis',
            distitbtn: '.btn-distit',
            invitebtn: '.btn-invite',
            chooseinvitebtn:'.btn-chooseinvite',
            recoverybth: '.btn-recovery',
            refreshinfobth: '.btn-refreshinfo',
            recyclefailbtn:'.btn-recyclefail',
            recycledesignatedbtn:'.btn-recycledesignated',
            showallbtn:'.btn-showall',
            hideallbtn:'.btn-hideall',
            dstinctbtn:'.btn-dstinct',
            yesterdaybtn:'.btn-yesterday',
            todaybtn:'.btn-today',
            thismonthbtn:'.btn-thismonth',
            lastmonthbtn:'.btn-lastmonth',
            twomonthsagobtn:'.btn-twomonthsago',
            beforeyesterdaybtn:'.btn-beforeyesterday',
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                toolbar.on('click', Table.config.refreshbtn, function () {
                  ...
                  //刷新任务信息按钮事件
                toolbar.on('click', Table.config.refreshinfobth, function () {
                      var ids = Table.api.selectedids(table);
                      url = 'https://ht.huikeyueke.cn/VQEegRMOwt.php/missions/updatedata'
                      Fast.api.ajax({
                      url:url,
                      data:{ids:ids}
                      });
                ...
                //回收未拨通按钮事件
                toolbar.on('click', Table.config.recyclefailbtn, function () {
                     var ids = Table.api.selectedids(table);
                   
                      url = 'https://ht.huikeyueke.cn/VQEegRMOwt.php/missions/recyclefail'
                      Fast.api.ajax({
                      url:url,
                      data:{ids:ids}
                      });
                ...
                // 指定时长回收
                toolbar.on('click', Table.config.recycledesignatedbtn, function () {
                     var ids = Table.api.selectedids(table);
                    var url = options.extend.recycledesignated_url+"?ids="+ids;
               
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('Recycledesignated'), $(this).data() || {});
                    // recycledesignated
                });
                //选择申请任务
                toolbar.on('click', Table.config.chooseinvitebtn, function () {
                     var ids = Table.api.selectedids(table);
                    var url = options.extend.chooseinvite_url+"?ids="+ids;
               
                   Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('Chooseinvite'), $(this).data() || {});
                ...
                toolbar.on('click', Table.config.exportbtn, function () {
                    ...
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('export'), $(this).data() || {});
                    
                });
                // 从用户信息添加任务按钮事件
                toolbar.on('click', Table.config.addtobtn, function () {
                         var that = this;
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.addto_url;
...
//   data:{ids:ids}
  ...
  //失败的回调
                ...
                //一键去重按钮
                toolbar.on('click', Table.config.dstinctbtn, function () {
                ...
                //申请任务按钮
                toolbar.on('click', Table.config.invitebtn, function () {
                
                Fast.api.ajax({
                url:'missionlists/invite',
                data:{}
                     },
                function(data, ret){
                ...
                function(data, ret){
                 ...
                 toolbar.on('click', Table.config.recoverybth, function () {
    
                    var that = this;
                    var ids = Table.api.selectedids(table);
                        Fast.api.ajax({
                          url:'missionlists/recoverymission',
                          data:{ids:ids}
                     }, function(data, ret){
                ...
                // 从通话记录添加任务按钮事件
                toolbar.on('click', Table.config.addtothebtn, function () {
             
             
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.addtothe_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }else{
...
//   data:{ids:ids}
  ...
  //失败的回调
                ...
                //分配人员到任务按钮事件
                toolbar.on('click', Table.config.distbtn, function () {
                    var ids = Table.api.selectedids(table);
                    var mynum =$('#mynum').val();
             var url = options.extend.dist_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }else{
  ...
  //失败的回调
                 ...
                 // 分配外呼任务按钮事件
                toolbar.on('click', Table.config.distitbtn, function () {
                    var ids = Table.api.selectedids(table);
               
                    var url = options.extend.distit_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }else{
  ...
  //失败的回调
    ...
    //分配查号任务按钮事件
    toolbar.on('click', Table.config.disttobtn, function () {
        
        var ids = Table.api.selectedids(table);
        var url = options.extend.distto_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }else{
  ...
  //失败的回调
                ...
                //踢出人员按钮事件
                toolbar.on('click', Table.config.getoutbtn, function () {
                    var ids = Table.api.selectedids(table);
                  
                    var url = options.extend.getout_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }else{
  ...
  //失败的回调
                ...
                //选人回收按钮事件
                toolbar.on('click', Table.config.recythisbtn, function () {
                    var ids = Table.api.selectedids(table);
                  
                    var url = options.extend.recythis_url;
                    if (url.indexOf("{ids}") !== -1) {
                        url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    }else{
  ...
  //失败的回调
                ...
                //设置任务隐藏按钮事件
                toolbar.on('click', Table.config.hideallbtn, function () {
                    var that = this;
                    var ids = Table.api.selectedids(table);
             var url = options.extend.hideall_url;
                    // if (url.indexOf("{ids}") !== -1) {
                    //     url = Table.api.replaceurl(url, {ids: ids.length > 0 ? ids.join(",") : 0}, table);
                    // }else{
  ...
  //失败的回调
                ...
                //昨日数据
                $(document).on('click', Table.config.yesterdaybtn, function () {
                    var that = this;
             
          
                    Fast.api.ajax({
                        url: 'dashboard/setdays',
                        data: {days:'yesterday'}
                    }, function () {
                 ...
                 //前日数据
                $(document).on('click', Table.config.beforeyesterdaybtn, function () {
                    var that = this;
             
          
                    Fast.api.ajax({
                        url: 'dashboard/setdays',
                        data: {days:'beforeyesterday'}
                    }, function () {
                ...
                //今日数据
                $(document).on('click', Table.config.todaybtn, function () {
                    var that = this;
                 ...
                 //当月数据
                $(document).on('click', Table.config.thismonthbtn, function () {
                    var that = this;
             
          
                    Fast.api.ajax({
                        url: 'dashboard/setmonth',
                 ...
                 //上月数据
                $(document).on('click', Table.config.lastmonthbtn, function () {
                    var that = this;
                ...
                //前月数据
                $(document).on('click', Table.config.twomonthsagobtn, function () {
                    var that = this;
             
          
                    Fast.api.ajax({
                        url: 'dashboard/setmonth',
                        data: {month:'two-months-ago'}
                    }, function () {
                ...
                //显示全部
                $(document).on('click', Table.config.showallbtn , function () {
                    var that = this;
                    var url = options.extend.showall_url;
            ...
            formatter: {
                ...
                datetimes: function (value, row, index) {
                    var datetimeFormat = typeof this.datetimeFormat === 'undefined' ? 'YYYY-MM-DD HH:mm:ss' : this.datetimeFormat;
                    if (isNaN(value)) {
                        return value ? Moment(value).format(datetimeFormat) : __('None');
                    } else {
                        return value ? Moment(parseInt(value) ).format(datetimeFormat) : __('None');
                    }
                },
